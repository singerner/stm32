...
int main(void)
{
	...
	while (1)
	{
			...
			if(bkey_flag)
			{
				...
				{
					case 0:
						for(cnt = 0; cnt < c_MaxColByte; cnt ++)
						{
								ucDispCMDDataBuff[cnt] = 0xFF;
						}			
						ucDispCMDDataBuff[cnt] = 0x00;
						break;						
					case 1:
						break;					
					case 2:
						for(cnt = 0; cnt < c_MaxColByte; cnt ++)
						{
								ucDispCMDDataBuff[cnt ++] = 0xB2;
								ucDispCMDDataBuff[cnt] = 0xD8;
						}				
						ucDispCMDDataBuff[cnt] = 0x00;
						break;
					case 3:
						for(cnt = 0; cnt < c_MaxColByte; cnt ++)
						{
								ucDispCMDDataBuff[cnt ++] = 0x7C;
								//ucDispCMDDataBuff[cnt ++] = 0x20;
								ucDispCMDDataBuff[cnt] = 0x20;
						}
						ucDispCMDDataBuff[cnt] = '\0';					
						break;
					default:
						break;
				}
			...
			{	
					...
					if(key_cnt == 3)
					{
							LED_ScrollDispHZ(1,1,ucDispCMDDataBuff);
					}
					else
					{
							LED_DispString(1,1,ucDispCMDDataBuff);
					}
...
{	
	  uint8_t ReadValue;
		...
		ReadValue = (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_7) ? Bit_SET : Bit_RESET);//1  Bit_SET,0	Bit_RESET	
		switch(ReadValue)
		{
			case Bit_RESET:
				//brightness = 0x1FFF;//0.68
				brightness = 0xEFF; //0.92mA
				break;
			case Bit_SET:
				brightness = 0x4FF; //0.92mA
				//brightness = 0xFFF;
				break;
			default:
				brightness = 0x4FF; //0.92mA
				//brightness = 0xFFF; //0.92mA
				break;
		}
}
...
void data_process(void)
{
	...
	if(checksum == Usart_ucDispCMDDataBuffTemp[++ num])
	{
		...
		{
			case FRONT:
				front_process(snowflag);
				break;
			case REAR:
				rear_process(snowflag);
				break;
			case SIDE:
				side_process(snowflag);
				break;
			default:
				break;
		}
...
void get_line_name(const uint8_t *src, uint8_t *dst)
{
	...
	for(ct = 0; src[ct] != '\0'; ct ++)
	{
		...
		uCharLow = src[++ ct];
		Code = (uint16_t)(uCharHigh << 8) + uCharLow;
		if((0xA3AA == Code) || (0xA1A1 == Code) || (0xA1F9 == Code))
		{
			continue;
		}
		else
...
{
	int i = 0;
 	SST25_R_BLOCK(0x180000, ucDispCMDDataBuff, 64);
	if((ucDispCMDDataBuff[0] != 0xFF) || (ucDispCMDDataBuff[1] != 0xFF))
	{
			for(i = 0; i < c_MaxColByte; i ++)
			{
					ucDispCMDDataBuff[i] = 0xFF;
			}
	}
	ucDispCMDDataBuff[c_MaxColByte] = 0x00;
	LED_OK = 1;
}


void get_line_from_flash1(void)
{
	uint8_t ct = 0, num = 0;
