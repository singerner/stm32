...
void ShT_LEDBoard_ColShiftOut(uint8_t Codata)
{
	...
	for (i = 0; i < 8; i++) 
	{
		...
		if (byte & 0x01)
			...
			GPIO_ResetBits(GPIOC, GPIO_Pin_3); //复位
		for(j=0;j<2;j++);
...
//	int i;
	...
	while(*pDispString_Head != '\0')
	{
		if(LED_OK)
		{
			...
			{
				//中文字符的处理
				ucCharLow = *pDispString_Head++;//中文字符是两个字节所以把uccharhigh和uccharlow合在一起
				uiHZCode =(((uint16_t)ucCharHigh)<<8)+ucCharLow;
				...
				if(bResultFlag)
					...
					GetHZDot(uiHZCode,ucHZDataBuff);//如果数组里面没找到就到字库里面去找
				
				for(uci=1; uci<5; uci++)//把找到的汉字放到缓存里面
				{
					if(LED_OK)
					{
						ucZMData = ucHZDataBuff[30-(16-uci)*2];//把buf里面的值写到uczmdata里面
