...
* @retval Staus
*/
int USBH_USR_MSC_Application(void)//从usb下载文件到指定路径并做分行处理   行尾家0A0D
{
  ...
  flash_run();
	...
	f_read(&file, buff, 2048, (void *)&numOfReadBytes);//numofReadBytes读的字节个数

	if(0 != numOfReadBytes)
	{
		for(num = 0, ct =1; num < numOfReadBytes; num ++)
		{
				if(num == numOfReadBytes - 2)//从零位开始最后一位是/0所以减2
				{
						if((0x0D == buff[num]) && (0x0A == buff[num + 1]))//线路信息结尾带0x0D,0x0A
						{
								line_count_t ++;
								num ++;
						}
						else//线路信息结尾不带0x0D,0x0A
						{
