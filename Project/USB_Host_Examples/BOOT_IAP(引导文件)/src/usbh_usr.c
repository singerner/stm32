...
unsigned long numOfReadBytes = 0;
extern void flash_run(void);
extern void flash_stop(void);
extern void flash_on(void);
extern void flash_off(void);
...
int USBH_USR_MSC_Application(void)
{
	...
	uint32_t flash_write_addr = ApplicationAddress;
	uint32_t cfg_write_addr = 0;
	flash_on();
	SPI_Flash_Init();
	flash_run();
	...
	EraseCounter = 0;
	if(f_open(&file, "led.bin", FA_READ) != FR_OK)
	{
		goto ziku_addr;
		//return 0;
	}
	for (EraseCounter = 0; EraseCounter < 20; EraseCounter++)
	{
		FLASH_ErasePage(FlashDestination + (PAGE_SIZE * EraseCounter));
	}
	do
	{
		f_read(&file, buf, PAGE_SIZE, (void *)&numOfReadBytes);
		if(numOfReadBytes !=0)
		{
			RamSource = (unsigned long )buf_ptr;
			for (i = 0;i < numOfReadBytes;i += 4)
			{
				/* Program the data received into STM32F10x Flash */
				FLASH_ProgramWord(flash_write_addr, *(unsigned long *)RamSource);
				if (*(uint32_t*)flash_write_addr != *(unsigned long *)RamSource)
				{
					numOfReadBytes = 0 ;//error
					/* End session */
				}
				flash_write_addr += 4;
				RamSource += 4;
...
ziku_addr:
	EraseCounter = 0;
	flash_write_addr = ZIKU_ADDR;
	FlashDestination = ZIKU_ADDR;

	if(f_open(&file, "ziku.bin", FA_READ) != FR_OK)
	{
		goto cfg_addr;
	}
	for(EraseCounter = 0; EraseCounter < 31; EraseCounter ++)
	{
		Block_Erase_64K(ZIKU_ADDR + (EraseCounter * SST25_PAGE));
	}
	do
	{
		f_read(&file, buf_ptr, 4096, (void *)&numOfReadBytes);
		if(numOfReadBytes != 0)
		{
			buf_ptr = buf;
			SST25_W_BLOCK(flash_write_addr, buf_ptr, numOfReadBytes);
			flash_write_addr += numOfReadBytes;
		}
	}while(numOfReadBytes != 0);
	if(f_close(&file) != FR_OK)
	{
...
cfg_addr:
	...
	if(f_open(&file, "cfg.txt", FA_READ) != FR_OK)
	{
		goto error;
	}
	Sector_Erase_4K(0);
	f_read(&file, cfg_buf, 4096, (void *)&numOfReadBytes);
	if(numOfReadBytes != 0)
	{
		buf[0] = ((cfg_buf[4] & 0xF) << 4) | (cfg_buf[5] & 0x0F);
		buf[1] = ((cfg_buf[13] & 0xF) << 4) | (cfg_buf[14] & 0x0F); 
		buf[2] = 0x0A;
		buf[3] = 0x0D;
		SST25_W_BLOCK(cfg_write_addr, buf, 4);
	}
